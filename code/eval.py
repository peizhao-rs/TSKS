...
eval_file_splits = eval_file.split('/')
metric_path = eval_file_splits[0]
for item in eval_file_splits[1:-1]:
    metric_path = os.path.join(metric_path, item)
metric_path = os.path.join(metric_path, 'metric.txt')
    
sents = []
for line in open(eval_file, encoding='utf8'):
    tk = line.strip().split("\t")
    if len(tk) < 2:
        continue
...
with open(metric_path, 'w', encoding='utf8') as fw:
    fw.write(output_str)
